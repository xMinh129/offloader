...
t1 = [-8.316023247148996, 7.390496535544129, -3.474414294194877, -6.395690968442976, 11.531621010864773,
      1.858046693937737, -4.296546083926734, -4.2682773335564255, -0.872604910687722, -0.8756237058277373,
      -0.853438060347419, -0.898599310298378, -0.8189118115025188, -4.533575569388763, -1.5477787218019972,
      -1.9371974403107177, 0.28197142216472704]
t2 = [-8.13742761413348, 7.389876116071173, -3.1102254397988474, -6.456356276212944, 11.547952825978772,
      1.6392209059910154, -4.299539590435781, -4.322184359470038, -11.71989314565338, -0.32054654848794095,
      -0.33743388408573816, -0.3610371576377754, -0.32351776967330614, -5.072370695588569, -1.954964968696801,
      -2.387751943244871, -0.12591017194451964]
path = "./server/models/input_example.csv"
...
def output(data, cny=False):
    ...
    if type(data) == str:
        ...
        df = data
    cust, df_transformed = preprocess(df)
    return json.loads(predict(cust, t, df_transformed).to_json())


def run_model():
    return output('input_example.csv')
